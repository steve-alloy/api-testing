...
const postEntity = async (url: string, token: string): Promise<void> => {
    ...
    const responseFromEntity = await axios.post(entityUrl, JSON.stringify(bodyObj), {
        headers: {
    ...
    const responseFromEntityWithDocument = await axios.put(entityUrlWithDocumentToken, file, {
        headers: {
